...
public class FakeHttpRequest extends FakeHttpMessage implements HttpRequest {
    ...
    private FakeHttpRequest(FakeHttpRequest request, HttpParameter parameter) {
        ...
        // update body
        this.body = parameters.stream()
                .filter(o -> o.type() == HttpParameterType.BODY)
                .map(o -> ((FakeParsedHttpParameter)o).toNameValueString())
    ...
    public String parameterValue(String name, HttpParameterType type) {
        return parameters.stream()
                .filter(parameter -> (parameter.name().equals(name) && (parameter.type() == type)))
                .findAny().map(ParsedHttpParameter::value).orElse(null);
    }

    @Override
    public boolean hasParameter(String name, HttpParameterType type) {
        return parameters.stream()
                .anyMatch(parameter -> (parameter.name().equals(name) && parameter.type() == type));
    }
