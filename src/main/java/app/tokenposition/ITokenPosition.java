...
public abstract class ITokenPosition {
  ...
  public ITokenPosition(HttpMessage httpMessage, boolean isRequest) {
  ...
  public void cleanJWTHeaders() {
    ...
    if (isRequest) {
      HttpRequest request = (HttpRequest) httpMessage;
      httpMessage = request.withRemovedHeader(Strings.JWTHeaderPrefix);
    }
    else
    {
      HttpResponse response = (HttpResponse) httpMessage;
      httpMessage = response.withRemovedHeader(Strings.JWTHeaderPrefix);
    }
  }
